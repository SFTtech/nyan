...
public:
	...
	public:
		 ...
		 */
		Keyframe(const order_t &time, const T &value)
			:
			time{time},
			value{value} {}

		const order_t time = std::numeric_limits<order_t>::infinity();
		T value = T();
	};
	 ...
	 * start the search.
	 */
	KeyframeIterator last(const order_t &time,
	 ...
	 * empty)
	 */
	KeyframeIterator last(const order_t &time) const {
	 ...
	 */
	KeyframeIterator insert(const order_t &time, const T&value) {
	 ...
	 */
	KeyframeIterator insert(const order_t &time, const T&value, const KeyframeIterator &hint) {
...
KeyframeContainer<T>::KeyframeContainer() {
	...
	this->container.push_back(Keyframe(-std::numeric_limits<order_t>::infinity(), T()));
}
 ...
 */
template <typename T>
typename KeyframeContainer<T>::KeyframeIterator KeyframeContainer<T>::last(const order_t &time, const KeyframeIterator &hint) const {
	...
	if (this->container.front().time > time) {
		...
		throw new Error{"rupture in spacetime detected, curve container is broken"};
	}
 ...
 */
template <typename T>
typename KeyframeContainer<T>::KeyframeIterator KeyframeContainer<T>::insert(const KeyframeContainer<T>::Keyframe &e, const KeyframeContainer<T>::KeyframeIterator &hint) {
 ...
 */
template <typename T>
typename KeyframeContainer<T>::KeyframeIterator KeyframeContainer<T>::erase_after(KeyframeContainer<T>::KeyframeIterator last_valid) {
 ...
 */
template <typename T>
typename KeyframeContainer<T>::KeyframeIterator KeyframeContainer<T>::erase(KeyframeContainer<T>::KeyframeIterator e) {
